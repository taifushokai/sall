...
def main
  ...
  if submit == "OK" && user_sentence != ""
    ...
    dbh.close
    time = Time::now - time0
    timestr = sprintf("%s (%.1f sec, model %s, in %d, out %d)", Time::now.strftime("%F %T"), time, model, insize, outsize)
  else # submit == "CLEAR"
    user_sentence = ""
    assistant_sentence = ""
  end
...
<html>
  ...
  <body style="text-align: left;">
    <form method="POST">
      ...
      <br />
      <textarea name="user_sentence" rows="6" readonly>#{user_sentence}</textarea>
      <br />
      ...
      <br />
      <textarea name="assistant_sentence" rows="12" readonly>#{assistant_sentence}</textarea>
      #{timestr}
      <br />
      <br />
      入力欄
      <br />
      <textarea name="user_sentence_new" rows="6" style="background: #ffe8c8;"></textarea>
