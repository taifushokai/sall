...
def main
  get_dbh()
=begin
  insert("INIT", "asst", "光子は電磁相互作用を媒介するゲージ粒子で、ガンマ線の正体であり γ で表されることが多い。")
  insert("INIT", "asst", "ウィークボソンは弱い相互作用を媒介するゲージ粒子で、質量を持つ。")
  insert("INIT", "asst", "Wボソンは電荷±1をもつウィークボソンで、ベータ崩壊を起こすゲージ粒子である。W+, W−で表され、互いに反粒子の関係にある。")
  insert("INIT", "asst", "Zボソンは電荷をもたないウィークボソンで、ワインバーグ＝サラム理論により予言され、後に発見された。Z0 と書かれることもある。")
  insert("INIT", "asst", "グルーオンは強い相互作用を媒介するゲージ粒子で、カラーSU(3)の下で8種類存在する（8重項）。")
  insert("INIT", "asst", "XボソンとYボソンはジョージ＝グラショウ模型において導入される未発見のゲージ粒子である。")
  insert("INIT", "asst", "重力子（グラビトン）は重力を媒介する未発見のゲージ粒子で、スピン2のテンソル粒子と考えられている。")
=end
  user_content = nil
  begin
    user_name = "Visiter"
    ...
    user_content = gets
    if user_content
      puts talk(user_name, user_content)
    end
    end while user_content
  puts
end

def talk(user_name, user_content)
  if $llm_client == nil
  ...
  insert(user_name, "user", user_content)
  insert(user_name, "asst", asst_content)
  return asst_content
end
